...
# Set seed and backend for reproducibility
torch.manual_seed(42)
if DEVICE == 'cuda':
    torch.set_float33_matmul_precision('high')
...
def create_vgg16(num_classes: int = 10) -> nn.Module:
    ...
    # Adapt the final classifier for the number of classes
...
# ───────── Pruning & Quantization Core Methods ─────────
def is_prunable_module(m: nn.Module) -> bool:
    """Checks if a module is prunable/quantizable (Conv2d or Linear)."""
    return isinstance(m, (nn.Conv2d, nn.Linear)) and hasattr(m, 'weight')


def l1_prune(module: nn.Module, amount: float):
    """Applies L1 unstructured pruning to a module's weight (for Conv2d/Linear only)."""
    if is_prunable_module(module) and amount > 0:
        prune.l1_unstructured(module, 'weight', amount)


def per_tensor_symmetric_quant(module: nn.Module, bits: int):
    """Applies pseudo-quantization to a module's weight (symmetric, per-tensor; for Conv2d/Linear only)."""
    if not is_prunable_module(module) or bits >= 32:
        return
...
def read_config_from_csv(csv_path: Path) -> Dict[str, Dict]:
    """Reads per-layer pruning and quantization config from a CSV file (one layer per row)."""
    if not csv_path.exists():
        raise FileNotFoundError(f"Configuration CSV not found at: {csv_path}")
    config = {}
    with open(csv_path, mode='r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            if '|' in layer_name:
                # Ensure per-layer configuration; skip merged names
...
def main():
    ...
    if BASELINE_ACC_JSON.exists():
        with open(BASELINE_ACC_JSON, 'r') as f:
            baseline_acc = json.load(f).get("vgg16", 0.0)
        print(f"Loaded baseline accuracy: {baseline_acc:.2f}%")
    else:
        print("Baseline accuracy JSON not found. Evaluating from scratch...")
        baseline_acc = evaluate(model, te_loader, "Evaluating Baseline")
        print(f"Calculated baseline accuracy: {baseline_acc:.2f}%")
    ...
    for layer_name, cfg in tqdm(strategy_config.items(), desc="Applying Pruning"):
        amount = cfg['pruning_ratio']
        if amount <= 0:
            continue
        if layer_name in modules:
            m = modules[layer_name]
            if is_prunable_module(m):
                l1_prune(m, amount)
                pruned_layers.append(layer_name)
            else:
                print(f"Skip pruning '{layer_name}' (not Conv/Linear).")
        else:
            print(f"Warning: Layer '{layer_name}' from CSV not found in model.")
    print(f"Applied pruning to {len(pruned_layers)} layers.")

    # Make pruning permanent by removing the masks
    ...
    for layer_name, cfg in tqdm(strategy_config.items(), desc="Applying Quantization"):
        ...
        if layer_name in modules:
            m = modules[layer_name]
            if is_prunable_module(m):
                per_tensor_symmetric_quant(m, bits)
                quantized_layers_count += 1
            else:
                print(f"Skip quant '{layer_name}' (not Conv/Linear).")
        else:
            print(f"Warning: Layer '{layer_name}' from CSV not found in model.")
