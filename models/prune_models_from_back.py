...
# All 6 models are enabled for this experiment
CFG = {
...
def main():
    ...
    # --- Read existing baselines from the CSV to avoid re-training ---
    existing_baselines = {}
    if CSV_PATH.exists():
    ...
    with CSV_PATH.open('a', newline='') as f_csv:
        ...
        for mdl_name in MODELS:
            ...
            # Check if a baseline for this model is already cached
            if mdl_name in existing_baselines:
                baseline_acc = existing_baselines[mdl_name]
                print(f'[SKIP base] Using cached baseline = {baseline_acc:.2f}%')
                # Dry run to initialize model parameters without training
                ...
                print(f'[TRAIN base] No cache found. Training baseline...')
                finetune(model, tr_loader, BASELINE_EPOCHS, BASE_LR, desc=f"{mdl_name}-base")
            ...
            # --- Main loop: Prune cumulatively from the last layer towards the first ---
            for depth_level in range(1, total_layers + 1):
                # Select the last 'depth_level' layers to prune in this step
                ...
                for (meth_name, prune_fn), ratio in product(PRUNE_METHODS.items(), PRUNE_AMOUNTS):
                    # Reset model to the clean baseline state before applying pruning
