...
PRUNE_AMOUNTS = [0.9, 0.8]  # Pruning ratios to be tested
BASELINE_EPOCHS = 8  # Epochs for initial model training
FINETUNE_EPOCHS = 1  # Epochs for fine-tuning after pruning
BASE_LR = 3e-4
FINETUNE_LR = 1e-4
WEIGHT_DECAY = 1e-4
NUM_WORKERS = 8
CSV_PATH = Path('results/pruned_by_layer_fine_turn_v1.csv')
...
def build_loaders(batch_train: int, batch_test: int) -> Tuple[DataLoader, DataLoader]:
    ...
    # Use performance optimizations for DataLoader
...
def create_model(name: str, num_cls: int = 10) -> nn.Module:
    ...
    # Adapt the final classifier layer for the target dataset (CIFAR-10)
...
def prunable_layers(model: nn.Module) -> List[str]:
    """Identifies and returns the names of layers suitable for pruning."""
    layers = []
    for n, m in model.named_modules():
        # Select Conv2d (excluding 1x1) and Linear layers for pruning
...
def finetune(model: nn.Module, loader: DataLoader,
    ...
    scaler = amp.GradScaler() if DEVICE == 'cuda' else None # For mixed-precision training
...
def main():
    ...
    with CSV_PATH.open('w', newline='') as f_csv:
        ...
        for mdl_name in MODELS:
            ...
            print(f'\n=== {mdl_name} (reuse-model) ===')
            # Create and train a baseline model from scratch
            model = create_model(mdl_name)
            finetune(model, tr_loader, BASELINE_EPOCHS, BASE_LR,
                     desc=f"{mdl_name}-base")
            baseline_acc = evaluate(model, te_loader)
            print(f'baseline_acc = {baseline_acc:.2f}%')
            n_params_total = sum(p.numel() for p in model.parameters() if p.requires_grad)

            # Save baseline weights to reset the model for each pruning experiment
            baseline_wts = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            layers = prunable_layers(model)
            tasks = list(product(PRUNE_METHODS.items(), layers, PRUNE_AMOUNTS))

            # Iterate over all pruning tasks (method, layer, ratio) for the current model
            for (meth_name, prune_fn), layer, ratio in tqdm(tasks, desc=f"{mdl_name}-tasks"):
                # Reset model to the baseline state before each experiment
                model.load_state_dict(baseline_wts, strict=True)
                module = dict(model.named_modules())[layer]
                prune_fn(module, amt=ratio)
                ...
                n_pruned = int(ratio * module.weight.numel())
                wr.writerow(dict(
                    model=mdl_name, layer=layer, method=meth_name, ratio=ratio,
                    baseline=baseline_acc, acc_no_ft=acc_no_ft, acc_ft=acc_ft,
                    inst_delta=acc_no_ft - baseline_acc,
                    ft_delta=acc_ft - baseline_acc,
                    **{'params_pruned (%)': round(100. * n_pruned / n_params_total, 4)}
                ))
                # Make the pruning permanent by removing the pruning mask
