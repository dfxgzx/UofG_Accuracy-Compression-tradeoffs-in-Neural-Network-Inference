import csv, math, time, warnings, json
...
# Try importing ptflops for FLOPs calculation
try:
    from ptflops import get_model_complexity_info

    PT_FLOPS_OK = True
except ImportError:
    warnings.warn('ptflops not installed; FLOPs will not be measured. Install via pip install ptflops')
    PT_FLOPS_OK = False

# ───────── Global Configuration ─────────
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'
QUANT_BITS = [8, 4]  # Quantization bit widths to test
FINETUNE_EPOCHS = 1  # Number of epochs to fine-tune after quantization
FINETUNE_LR = 1e-4  # Learning rate for fine-tuning
WEIGHT_DECAY = 1e-4  # Weight decay during fine-tuning
NUM_WORKERS = 8  # Number of workers for DataLoader
CSV_PATH = Path('results/quant_sensitivity_full_metrics.csv')  # Output CSV
CKPT_DIR = Path('checkpoints')  # Directory for pre-trained model checkpoints

# Predefined baseline accuracy and checkpoint paths for each model
BASELINE_INFO = {
...
# Recommended batch sizes for training and testing per model
CFG = {
...
def build_loaders(bt_train: int, bt_test: int) -> Tuple[DataLoader, DataLoader]:
    # Define image transforms for training and testing on CIFAR-10
...
def create_model(name: str, num_cls: int = 10) -> nn.Module:
    # Create a torchvision model and adjust its classifier head for CIFAR-10
...
def quantizable_layers(model: nn.Module) -> List[str]:
    # Return names of layers suitable for quantization (Conv2d and Linear layers)
    layers = []
...
def finetune(model: nn.Module, loader: DataLoader, epochs: int):
    # Fine-tune the model for a few epochs on training data
...
# These perform in-place quantization on weight tensors

def _linear_quant_per_tensor(weight: torch.Tensor, bits: int, symmetric: bool):
    # Quantize entire tensor using a single scale (symmetric or asymmetric)
...
def _linear_quant_per_channel(weight: torch.Tensor, bits: int):
    # Quantize each output channel separately using independent scales
    qmax = 2 ** (bits - 1) - 1
    with torch.no_grad():
        w = weight.detach()
        out_dim = w.shape[0]
...
# Quantization method wrappers for applying to module
...
def inspect_model(model: nn.Module, bits: int) -> Tuple[float, float, float]:
    # Estimate model size (MB), FLOPs (G), and latency (ms)
...
def main():
    ...
    with CSV_PATH.open('w', newline='') as f_csv:
        ...
        for mdl in MODELS:
            ...
            assert info, f'Baseline info for {mdl} missing'
            ckpt_path = Path(info['ckpt'])
            assert ckpt_path.exists(), f'Checkpoint {ckpt_path} not found. Run baseline_benchmark.py first'
